...
export const create = async (user, password) => {
    ...
    try {
        const res = await db.query(`INSERT INTO "user"(name, email, password) VALUES($1, $2, $3) RETURNING id, name, email, wins, losses, draws, earnings`, [user.name, user.email || null, password]);
        return res.rows[0];
    }
    catch (err) {
        console.log(err);
...
export const findById = async (id) => {
    ...
    try {
        const res = await db.query(`SELECT id, name, email, wins, losses, draws, earnings FROM "user" WHERE id=$1`, [id]);
        if (res.rowCount) {
            return res.rows[0];
        }
        else
...
export const findByNameEmail = async (user, includePassword = false, limit) => {
    ...
    if (!user) {
        try {
            const res = await db.query(`SELECT id, name, email, wins, losses, draws, earnings FROM "user" LIMIT $1`, [limit ?? 10]);
            return res.rows;
        }
        catch (err) {
            console.log(err);
